    ...
    Submission.find({'user_email': req.params.email}, (err, submissions) => {
        if (err){
            ...
            if(submissionFound){
                ...
                res.status(500).send({Status: 500, Message: "No submissions from this user at the moment"});
            }
        }
    });   
}
        ...
        if (err){
            ...
            course.assignments.forEach((assignment) => {
                if(assignment.assignment_num == req.params.assignment_num){
                    ...
                    if(!submissionFound) res.status(500).send({Status: 500, Message: "No submissions from this user"});
                }
            });
        } 
    });   
                ...
                if(assignment.assignment_num == req.params.assignment_num){
                    ...
                    async.parallel(submissionList, function(err, result) {
                        ...
                        if(submissionFound){
                            ...
                            res.status(500).send({Status: 500, Message: "No submissions at the moment"});
                        }
                    });
                }
            });
