...
export function submitCSV(fileSelector, state) {
  var formDataCSV = new FormData();
  formDataCSV.append("complete", state.completeCSV);
  formDataCSV.append("file", fileSelector.files[0]);
  ...
  return function(dispatch) {
    ...
    return callApiWithFiles("courses/" + state.course_num + "/importRoster", formDataCSV).then(formDataCSV => {
